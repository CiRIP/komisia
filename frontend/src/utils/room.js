...
import { getProtooUrl } from './url';
import * as persistentStateManger from './persistence';
...
export default class RoomClient {
	...
	async disableAudioOnly() {
		...
		if (
			...
			this._produce &&
			(persistentStateManger.getDevices() || {}).webcamEnabled
		) {
			this.enableWebcam();
		}
	...
	async changeDisplayName(displayName) {
		...
		// Store in cookie.
		persistentStateManger.setUser({ displayName });
	...
	async _joinRoom() {
		...
		try {
			...
			if (this._produce) {
				...
				const devicesCookie = persistentStateManger.getDevices();
